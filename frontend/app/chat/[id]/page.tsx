...
export default function ChatDetailPage() {
  ...
  const canSend = useMemo(
    () => input.trim().length > 0 && !sending && !isStreaming,
    [input, sending, isStreaming]
  );
  ...
  async function sendMessage() {
    ...
    ChatAPI.streamMessage(
      ...
      (chunk) => {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m._id === tempAIId ? { ...m, content: streamBuffer.current } : m
          )
        );
      },
      async () => {
  ...
  const saveEdit = async (msgId: string, newContent: string) => {
    try {
      ...
      setEditingId(null);
      await ChatAPI.editMessage(token!, msgId, newContent); // backend update
    } catch (err) {
  ...
  try {
    // Instantly remove message from UI
    setMessages((prev) => prev.filter((m) => m._id !== msgId));

    // Only call backend if it's a real DB message
    if (!msgId.startsWith("temp")) {
      await ChatAPI.deleteMessage(token!, msgId);
    }
  } catch (err: any) {
