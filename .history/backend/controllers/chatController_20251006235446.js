import { Chat, Message } from './models.js'; // Imports Mongoose models
import LlmService from './LlmService.js'; // Imports the simulated LLM service
...
const createChat = async (req, res) => {
    try {
        const newChat = await Chat.create({
            userId: req.user._id, // User ID comes from the JWT middleware (req.user)
...
/**
 * @desc    Send a new message to a chat and trigger the async LLM response simulation
 * @route   POST /api/chats/:id/send
 * @access  Private
 */
const sendMessage = async (req, res) => {
    ...
    try {
        ...
        // 3. Update the chat's title if this is the first message
        ...
        // 4. IMMEDIATE RESPONSE (HTTP 202 Accepted)
        // This is crucial: the client receives the user's message immediately and can
        // display it while the AI response is being processed in the background.
        res.status(202).json({
        ...
        // =========================================================================
        // 5. ASYNCHRONOUS AI RESPONSE SIMULATION (Non-blocking)
        // This block runs after the response is sent to the client.
        // =========================================================================
        ...
        // Note: Errors here cannot be sent to the client as the request is already closed.
    }
};
