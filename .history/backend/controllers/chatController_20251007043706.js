...
export const getUserChats = async (req, res) => {
    try {
        const search = req.query.q?.trim(); // optional search term

        // Base query: only return chats belonging to the authenticated user
        ...
        // Fetch and sort chats (newest first)
...
export const editMessage = async (req, res) => {
  try {
    ...
    // Only allow edit if user owns the message
    if (msg.userId && msg.userId.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: "Not authorized to edit this message" });
    }

    msg.content = newContent || msg.content;
    await msg.save();

    res.status(200).json({ message: "Message updated successfully", msg });
  } catch (error) {
...
export const deleteMessage = async (req, res) => {
  try {
    ...
    // Only allow delete if user owns the message (skip check for AI/system messages)
    if (msg.userId && msg.userId.toString() !== req.user._id.toString()) {
