...
class LlmService {
  ...
  async *getSimulatedStream(userMessage) {
    const fullResponse = `Simulated AI reply: "${userMessage}" — sent in chunks to simulate streaming.`;
    const words = fullResponse.split(" ");

    for (let i = 0; i < words.length; i++) {
      await new Promise((resolve) => setTimeout(resolve, 200)); // small delay per chunk
