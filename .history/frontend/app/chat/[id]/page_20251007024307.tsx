...
import { Send, CornerUpLeft } from "lucide-react"; // Import necessary icons
...
export default function ChatDetailPage() {
    ...
    const fetchMessages = useCallback(async () => {
        // IMPROVEMENT: Added explicit check that id is a non-empty string
        if (!token || typeof id !== 'string' || id.length === 0) return;
        try {
    ...
    useEffect(() => {
        // IMPROVEMENT: Added explicit check that id is a non-empty string before fetching
        if (isAuthenticated && token && typeof id === 'string' && id.length > 0) {
            fetchMessages();
        }
    }, [fetchMessages, isAuthenticated, token, id]);
    ...
    async function send() {
        if (!token || typeof id !== 'string' || id.length === 0 || !canSend) return; // Added check for id validity
        setSending(true);
        setError(null);
        ...
        try {
            ...
            // Create a temporary ID for the optimistic message update
            const tempId = `temp-${Date.now()}`; 
            setMessages(prev => [...prev, { _id: tempId, content: userMessageContent, role: 'user', timestamp: new Date().toISOString() } as Message]);
            
            await ChatAPI.sendMessage(token, id, userMessageContent);
            ...
            pollRef.current = setInterval(async () => {
                ...
                // Smart Polling Check: is AI reply received?
                const isAiReplyReceived = list.length >= initialMessageCount && list[list.length - 1].role === 'ai';
                
                // Replace the temporary user message with the definitive list from the server
                setMessages(list);

                if (isAiReplyReceived || Date.now() - startedAt > MAX_POLL_TIME) {
                    if (pollRef.current) {
                        clearInterval(pollRef.current);
                        pollRef.current = null; // Mark polling as stopped
                    }
    ...
    return (
        // Main container: Dark background, 100vh height, flex layout (no sidebar in this view)
