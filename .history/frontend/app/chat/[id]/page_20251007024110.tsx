...
export default function ChatDetailPage() {
    ...
    // 1. Auth Guard: Redirects if not authenticated
    useEffect(() => {
    ...
    // 2. Memoized function to handle fetching messages
    ...
    // 4. Scroll to bottom whenever messages are updated
    useEffect(() => {
        messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
    }, [messages]);

    // 5. FIX: Cleanup effect for the polling interval
    // This stops the background timer when the component unmounts (e.g., when clicking 'Back')
    useEffect(() => {
        return () => {
            if (pollRef.current) {
    ...
    async function send() {
        ...
        // Clear any existing polling interval before sending a new message
        if (pollRef.current) {
            clearInterval(pollRef.current);
            pollRef.current = null;
        }
        ...
        <div className="min-h-screen flex flex-col bg-gray-900 text-gray-200 font-inter">
            ...
            <header className="flex-shrink-0 flex items-center p-4 border-b border-gray-700 bg-gray-800">
                <button 
                    ...
                    // Ensures clean client-side navigation to the list page
